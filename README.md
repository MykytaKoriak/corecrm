...
- Единый inbox для сообщений Telegram/Instagram/телефонии/ручного ввода.
- Звонки: входящие, пропущенные, запись звонка и автоматическая задача на перезвон.
- Доставка: базовая ТТН Новой Почты, статусы посылки и возвраты.
- Заявки чат-ботов: сбор контактов и создание клиента/сделки.
- Файлы клиентов и заказов.
- Товары внутри сделки с пересчетом суммы и переносом в заказ.
- Настройки: этапы, пользовательские поля, заготовки интеграций, шаблоны документов.
- Аналитика по продажам, конверсии, менеджерам, источникам и потерянным сделкам.
- REST API под `/api/`.
...
11. Проверить inbox `/inbox/`, звонки `/calls/`, доставку `/shipments/`.
12. Проверить аналитику `/analytics/` и шаблоны документов `/document-templates/`.
13. Проверить историю в `/settings/`.
14. Проверить API `/api/clients/`.
